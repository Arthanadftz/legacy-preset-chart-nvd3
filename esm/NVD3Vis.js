...
import { t, isDefined, getTimeFormatter, smartDateVerboseFormatter, getNumberFormatter, NumberFormats, CategoricalColorNamespace } from '@superset-ui/core';
...
import { cleanColorInput, computeBarChartWidth, computeYDomain, computeStackedYDomain, drawBarValues, generateBubbleTooltipContent, generateCompareTooltipContent, generateMultiLineTooltipContent, generateRichLineTooltipContent, generateTimePivotTooltip, generateTooltipClassName, generateAreaChartTooltipContent, getMaxLabelSize, getTimeOrNumberFormatter, hideTooltips, tipFactory, tryNumify, removeTooltip, setAxisShowMaxMin, stringifyTimeRange, wrapTooltip } from './utils';
import { annotationLayerType, boxPlotValueType, bulletDataType, categoryAndValueXYType, rgbObjectType, numericXYType, numberOrAutoType, stringOrObjectWithLabelType } from './PropTypes';
...
const {
  getColor,
  getScale
} = CategoricalColorNamespace; // Limit on how large axes margins can grow as the chart window is resized
...
const propTypes = {
  data: PropTypes.oneOfType([PropTypes.arrayOf(PropTypes.oneOfType([// pie
  ...
  vizType: PropTypes.oneOf(['area', 'bar', 'box_plot', 'bubble', 'bullet', 'compare', 'column', 'dist_bar', 'line', 'line_multi', 'time_pivot', 'pie', 'dual_line']),
  ...
  pieLabelType: PropTypes.oneOf(['key', 'value', 'percent', 'key_value', 'key_percent', 'key_value_percent']),
...
function nvd3Vis(element, props) {
  ...
  const drawGraph = function drawGraph() {
    ...
    switch (vizType) {
      ...
      case 'dist_bar':
        chart = nv.models.multiBarChart().showControls(showControls).reduceXTicks(reduceXTicks).groupSpacing(0.1); // Distance between each group of bars.
      ...
      case 'bubble':
        ...
        chart.tooltip.contentGenerator(d => generateBubbleTooltipContent({
          point: d.point,
          entity,
          xField,
          yField,
    ...
    if (isTimeSeries) {
      xAxisFormatter = getTimeFormatter(xAxisFormat); // In tooltips, always use the verbose time format
    ...
    if (chart.yAxis && chart.yAxis.tickFormat) {
      if ((contribution || comparisonType === 'percentage') && (!yAxisFormat || yAxisFormat === NumberFormats.SMART_NUMBER || yAxisFormat === NumberFormats.SMART_NUMBER_SIGNED)) {
    ...
    if (vizType === 'time_pivot') {
      ...
      chart.useInteractiveGuideline(true);
      chart.interactiveLayer.tooltip.contentGenerator(d => generateTimePivotTooltip(d, xAxisFormatter, yAxisFormatter));
    ...
    if (isVizTypes(['compare'])) {
      chart.interactiveLayer.tooltip.contentGenerator(d => generateCompareTooltipContent(d, yAxisFormatter));
    }
    ...
    if (isVizTypes(['dual_line', 'line_multi'])) {
      ...
      chart.useInteractiveGuideline(true);
      chart.interactiveLayer.tooltip.contentGenerator(d => generateMultiLineTooltipContent(d, xAxisFormatter, yAxisFormatters));
    ...
    if (chart.yAxis !== undefined || chart.yAxis2 !== undefined) {
      ...
      if (isTimeSeries && annotationData && activeAnnotationLayers.length > 0) {
        ...
        const timeSeriesAnnotations = activeAnnotationLayers.filter(layer => layer.annotationType === ANNOTATION_TYPES.TIME_SERIES).reduce((bushel, a) => bushel.concat((annotationData[a.name] || []).map(series => {
          if (!series) {
            return {};
          }
      ...
      if (isTimeSeries && activeAnnotationLayers.length > 0) {
        // Formula annotations
        const formulas = activeAnnotationLayers.filter(a => a.annotationType === ANNOTATION_TYPES.FORMULA).map(a => ({ ...a,
          formula: mathjsParse(a.value)
        }));
        let xMax;
        let xMin;
        let xScale;
        ...
        if (annotationData) {
          // Event annotations
          activeAnnotationLayers.filter(x => x.annotationType === ANNOTATION_TYPES.EVENT && annotationData && annotationData[x.name]).forEach((config, index) => {
            const e = applyNativeColumns(config); // Add event annotation layer

            const annotations = d3.select(element).select('.nv-wrap').append('g').attr('class', `nv-event-annotation-layer-${index}`);
          ...
          activeAnnotationLayers.filter(x => x.annotationType === ANNOTATION_TYPES.INTERVAL && annotationData && annotationData[x.name]).forEach((config, index) => {
            const e = applyNativeColumns(config); // Add interval annotation layer

            const annotations = d3.select(element).select('.nv-wrap').append('g').attr('class', `nv-interval-annotation-layer-${index}`);
            const aColor = e.color || getColor(cleanColorInput(e.name), colorScheme);
            const tip = tipFactory(e);
            const records = (annotationData[e.name].records || []).map(r => {
              const timeValue = new Date(moment.utc(r[e.timeColumn]));
